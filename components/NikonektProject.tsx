...
import NikonektProjectScreen from "../imports/NikonektProjectScreen";
import ProjectNavigation from "./ProjectNavigation";

interface NikonektProjectProps {
  onBack: () => void;
  onProjectClick?: (projectName: string) => void;
}

export default function NikonektProject({ onBack, onProjectClick }: NikonektProjectProps) {
  useEffect(() => {
  ...
  // Add click handler to back button and next button
  useEffect(() => {
    ...
    const handleNextClick = (e: Event) => {
      const target = e.target as HTMLElement;
      // Look for the next button area that contains "next" text and the thumbnail
      const nextText = target.textContent?.toLowerCase();
      if (nextText?.includes('next') || target.closest('[data-name="THUMB SITES"]')) {
        e.preventDefault();
        e.stopPropagation();
        // Navigate to Cool Sites project
  ...
  return (
    <div className="min-h-screen bg-[#111111] relative">
      <style>{`
        ...
        [data-name*="NIKONEKT"] .absolute[style*="bottom-[-"] {
          display: none !important;
        }
      `}</style>
      
      <ProjectNavigation 
        projectName="NIKONEKT" 
        projectCategory="SOCIAL NETWORKING"
        onBack={onBack}
      />
      
      <NikonektProjectScreen />
    </div>
  );
}